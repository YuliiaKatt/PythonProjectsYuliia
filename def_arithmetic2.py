def arithmetic2(number1, number2, operator):
    ...
    Перебирая словарь, находим совпадения ключа и заданного оператора.
    Если ключ и оператор совпадают, возвращается значение ключа (в котором производится математическое вычисление).
    Если совпадений нет, выводится строка "Неизвестная операция"
